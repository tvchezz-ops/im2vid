...
async def handle_nowpayments_webhook(request: web.Request) -> web.Response:
    """Handle verified NOWPayments IPN callbacks."""
...
async def handle_stars_wallet_webhook(request: web.Request) -> web.Response:
    """Handle verified Stars wallet bot callbacks."""
    from app.services.stars_wallet_webhook import process_stars_wallet_webhook

    raw_body = await request.read()
    status, payload = await process_stars_wallet_webhook(
        raw_body=raw_body,
...
def create_media_app(bot: Bot | None = None) -> web.Application:
    ...
    app.router.add_post("/webhooks/stars-wallet", handle_stars_wallet_webhook)
